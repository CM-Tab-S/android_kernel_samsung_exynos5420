...
#define MALI_T6XX_DEFAULT_CLOCK 400000000
...
static int kbase_platform_power_clock_init(kbase_device *kbdev)
{
	...
	/* Turn on G3D power */
	__raw_writel(0x7, EXYNOS5412_G3D_CONFIGURATION);

	/* Wait for G3D power stability for 1ms */
	timeout = 10;
	while ((__raw_readl(EXYNOS5412_G3D_STATUS) & 0x7) != 0x7) {
		if(timeout == 0) {
			/* need to call panic  */
	...
	if (IS_ERR(platform->aclk_g3d)) {
		OSK_PRINT_ERROR(OSK_BASE_PM, "failed to clk_get [aclk_g3d]\n");
		goto out;
	}

	aclk_g3d_sub = clk_get(dev, "aclk_g3d_sub");
	if (IS_ERR(aclk_g3d_sub)) {
		OSK_PRINT_ERROR(OSK_BASE_PM, "failed to clk_get [aclk_g3d_sub]\n");
		goto out;
	}

	clk_set_rate(platform->aclk_g3d, 100000000);
	ret = clk_set_parent(aclk_g3d_sub, platform->aclk_g3d);
	if (ret < 0) {
		OSK_PRINT_ERROR(OSK_BASE_PM, "failed to clk_set_parent [aclk_g3d_sub]\n");
		goto out;
	}

	clk_set_rate(platform->aclk_g3d, MALI_T6XX_DEFAULT_CLOCK);
	if (IS_ERR(platform->aclk_g3d)) {
		OSK_PRINT_ERROR(OSK_BASE_PM, "failed to clk_set_rate [aclk_g3d] = %d\n", MALI_T6XX_DEFAULT_CLOCK);
		goto out;
	}
...
{
	return ((__raw_readl(EXYNOS5412_G3D_STATUS) & 0x7) == 0x7) ? 1 : 0;
}
...
static int kbase_platform_power_off(void)
{
	...
	/* Turn off G3D  */
	__raw_writel(0x0, EXYNOS5412_G3D_CONFIGURATION);

	/* Wait for G3D power stability */
	timeout = 1000;

	while (__raw_readl(EXYNOS5412_G3D_STATUS) & 0x7) {
		if(timeout == 0) {
			/* need to call panic */
...
static ssize_t show_clock(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "Current aclk_g3d[G3D_BLK] = %dMhz", clkrate/1000000);
...
static ssize_t show_vol(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret += snprintf(buf+ret, PAGE_SIZE-ret, "Current operating voltage for mali t6xx = %d, 0x%x", vol, __raw_readl(EXYNOS5412_G3D_STATUS));
